...
const Thumbnails = () => {
  ...
  const paraphraseParagraph = async (paragraph) => {
    const options = {
      ...
      headers: {
        ...
        'X-RapidAPI-Key': '7dac7b086cmsh082b7f85bdbcb90p111240jsnb1680cc8378d',
  ...
  useEffect(() => {
    const paraphrasePdf= async () => {
      paraphrasedBio = await paraphraseParagraph(location.state.userData.user.bio);
    }
    paraphrasePdf();
  },[])
  ...
  const generatePdf1 = async () => {
    ...
    // let paraphrasedBio = await paraphraseParagraph(location.state.userData.user.bio);
    ...
    doc.setFontSize(subheadingFontSize);
    doc.setTextColor("#3B3B3B");
    doc.text("\u2022 Enhanced Bio:", 15, 130);
    doc.setFontSize(bulletFontSize);
    doc.setTextColor("#000000");
    const paraphrasedLines = doc.splitTextToSize(paraphrasedBio, pageWidth - 30, { lineHeightFactor: lineHeight });
    doc.setFont("Montserrat", "normal");
    doc.text(paraphrasedLines, 15, 140, { textAlign: "justify" });
  ...
  const generatePdf2 = async () => {
    ...
    doc.setFontSize(subheadingFontSize);
    doc.setTextColor(255, 171, 74);
    doc.setFont("OpenSans", "bold"); // Change font to Montserrat
    doc.text("Enhanced Bio", centeredX, 170, { align: "center" });
    doc.setLineWidth(0.3);
    doc.setDrawColor("#E7E7E7");
    doc.line(15, 174, pageWidth - 15, 174);
    doc.setFontSize(bulletFontSize);
    doc.setTextColor(43, 46, 131);
    const paraphrasedLines = doc.splitTextToSize(paraphrasedBio, pageWidth - 35, { lineHeightFactor: lineHeight });
    doc.setFont("Montserrat", "normal");
    doc.text(paraphrasedLines, 25, 184, { textAlign: "justify" });
  ...
  const generatePdf3 = async () => {
    ...
    // Paraphrased Bio
    doc.setFillColor(255, 0, 0, 0.5);
    doc.rect(15, 175, pageWidth - 30, 7, 'F');
    doc.setFontSize(subheadingFontSize);
    doc.setTextColor(255, 255, 255);
    doc.setFont("OpenSans", "bold");
    doc.text("Enhanced Bio", centeredX, 180, { align: "center" });
    doc.setLineWidth(0.3);
    doc.setDrawColor("#E7E7E7");
    doc.line(15, 184, pageWidth - 15, 184);
    doc.setFontSize(bulletFontSize);
    doc.setTextColor(43, 46, 131);
    const paraphrasedLines = doc.splitTextToSize(paraphrasedBio, pageWidth - 35, { lineHeightFactor: lineHeight });
    doc.setFont("OpenSans", "normal");
    doc.text(paraphrasedLines, 25, 194, { textAlign: "justify" });
